...
def Render(asset, eye_type, deg, iscenter, color, body_hsl, foot_hsl, result_mult, isband): # Render the tee from asset image
    ...
    threshold = 1
    if isband: # Band makes bot return a band of multiple renders with different eyes (Idea from official DDNet Bot)
        threshold = 5
        band_padding = int(112 * m) # Space between each tee
        band_width = int(((128 * m) + band_padding) * 2.5)
        band_height = int(128 * m)
        band_img = Image.new("RGBA", (band_width, band_height), (0, 0, 0, 0)) # 6 Different Eye Types
    ...
    while i < threshold:
        ...
        if isband:
            band_img.paste(skin, (band_x, 0), skin)
            if i < 6:
                band_x += band_padding
    ...
    result_img = result_img.resize((int(result_img.width * result_mult), int(result_img.height * result_mult))) # Resize Final Image
    result_img.save("C:/users/broso/Desktop/BandSkin.png")
    with BytesIO() as img_binary: # This is to be able to send the image as a message without having to save it locally
        result_img.save(img_binary, 'PNG')
        img_binary.seek(0)
...
class TeeRender(commands.Cog):
    ...
    @tree.choices( # Set up the options
        eyes=[
            ...
            Choice(name="Surprised", value=4),
            Choice(name="Dead", value=5)
        ],
    ...
    async def tee(self,
        ...
        render_param = functools.partial(Render, asset, eyes, deg, iscenter, iscolor, body_hsl, feet_hsl, result_mult, isband) # Pack all parameters into a single variable
        async with client:
